...
graphml_for_cytoscape <- list.files(path = "../results/LSA_tables", pattern = "network_triplets.*.graphml", all.files = FALSE,
                                    full.names = FALSE, recursive = FALSE,
                                    ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE)

#graphml_for_cytoscape <-graphml_for_cytoscape[21]
...
for (i in 1:length(graphml_for_cytoscape)){
 ...
 env_variable <- gsub(".*with(.*)\\.graphml","\\1", graphml_for_cytoscape[i])
 V(graph_file)$color <- ifelse(V(graph_file)$name %in% env_variable, "red", "yellow") #colour env red and otus blue
 plot(graph_file, vertex.size=5, edge.arrow.size=0.0, vertex.label.cex=0.5)
 pdf(file=paste0("../figures/network_triplets", env_variable,".pdf"))
 plot(graph_file, vertex.size=5, edge.arrow.size=0.0, vertex.label.cex=0.5)
 dev.off()
 ## cytoscape mushign all of them together. 
...
#saveCytoscapeSession(filepath="../figures/networks_triplets_and_with_modules.cys")