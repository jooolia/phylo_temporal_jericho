...
correlation_and_print_table <- function (group_sums, table_print_name) {
  
  VC_dates <- Jericho_data$Date[match(group_sums$VC, Jericho_data$VC)]
  params_Jericho_RdRp <- subset(params_Jericho, Date %in% VC_dates)

  spearman <- rcorr(as.matrix(group_sums[,-(1:2)]), as.matrix(params_Jericho_RdRp[,-1]),type="spearman") 
...
correlation_and_print_table(MPL_group_sums, "../results/RdRP_groups_cor.csv")

# gp23

correlation_and_print_table(gp23_group_sums, "../results/gp23_groups_cor.csv")
### 18s

correlation_and_print_table(S18_ord_sums, "../results/S18_orders_cor.csv")

### compare groups to S18 and env to MPL ####
correlation_between_amplicons_and_print_table <- function (group_sums_1,group_sums_2, table_print_name) {
 group_sums_1$Date <- Jericho_data$Date[match(group_sums_1$VC, Jericho_data$VC)]
...
union_vcs <- intersect(group_sums_2$VC, group_sums_1_with_env$VC)

group_sums_2_jer <- droplevels(subset(group_sums_2, group_sums_2$VC %in% union_vcs))

group_sums_1_with_env_jer <- droplevels(subset(group_sums_1_with_env, group_sums_1_with_env$VC%in% union_vcs))

spearman <- rcorr(as.matrix(group_sums_2_jer[,-(1:2)]), as.matrix(group_sums_1_with_env_jer[,-(1:2)]),type="spearman") 
correlations <- spearman$r
p <- spearman$P
...
significant_cor <- dcast(filtered_data_groups, Var1 ~Var2, value.var="cor_with_stars")
write.csv(significant_cor, file=table_print_name)
}

correlation_between_amplicons_and_print_table(S18_ord_sums, MPL_group_sums, "../results/RdRp_groups_with_18S_orders_cor.csv")

### compare groups to S18 and env to gp23 ####
correlation_between_amplicons_and_print_table(S18_ord_sums, gp23_group_sums, "../results/gp23_groups_with_18S_orders_cor.csv")

### 16S and gp23
correlation_between_amplicons_and_print_table(S16_ord_sums, gp23_group_sums, "../results/gp23_groups_with_16S_orders_cor.csv")
