...
#head $normalized_otu_table_top_10 -n 1 | sed s/\"//g > top_10_OTUs.txt
	...
	sed 's/Lib_pool\([0-9]\+\)/Lib-pool\1test/g' ../results/$file"_"$OTU"like_reads_aligned_all_times_edit.fasta" | sed 's/_//g' | sed 's/test/_/g' > ../results/$file"_"$OTU"like_reads_aligned_all_times_new_names.fasta" 	

	head ../results/$file"_"$OTU"like_reads_aligned_all_times_new_names.fasta"	

	entropy-analysis ../results/$file"_"$OTU"like_reads_aligned_all_times_new_names.fasta" --quick
