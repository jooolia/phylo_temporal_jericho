...
## Date modified: 10 September 2014
library(ggplot2)
library(scales)
library(reshape2)
library(gridExtra)
...
irradiation_data <- read.csv("../data/ibis_shortwave_irradiance_2010_2011_date_fixed.csv",row.names =
                              1)
...
maxWidth = grid::unit.pmax(
 gA$widths[2:3], gB$widths[2:3],
 gC$widths[2:3], gD$widths[2:3],
 gE$widths[2:3], gF$widths[2:3],
 gG$widths[2:3], 
 gH$widths[2:3], gI$widths[2:3]
)
...
environmental_plot_with_se <-
 function(data_frame, data_frame_mean, data_frame_se, ylimits,ylabel) {
  ...
  param_plot_line <- ggplot(param_with_se,
   ...
   geom_vline(xintercept = as.numeric(c(as.Date("2010-03-22"), 
                                        as.Date("2010-06-22"), 
                                        as.Date("2010-09-22"),
                                        as.Date("2010-12-22"),
                                        as.Date("2011-03-22"),
              ...
              colour="grey",
              size=2)+
   geom_vline(xintercept = as.numeric(as.Date("2011-04-08")),
              colour="green",
              size=2)+
   geom_line(colour=line_colour, size=1.5) +
   geom_point(colour = "grey50") +
...
bacterial_abundance_full_dataset_with_se <- environmental_plot_with_se(Jericho_data,
                                                                       Jericho_data[,"Average_bacterial_abundance"],Jericho_data[,"Standard_error_bacterial_abundance"],
                                                                       c(1,60000000),
                                                                       "Bacterial abundance\n (cells/mL)"
)+ 
 scale_y_continuous(labels = fancy_scientific,
...
pdf(paste0(figures_dir,"Jericho_full_panel_chart_with_se.pdf"), width = 15, height = 15, onefile = FALSE
)
grid.arrange(gA, gB, gC, gD,
             gE, gF, gG, gH,
             #gI,
...
pdf(paste0(figures_dir,"Jericho_env_with_se%03d.pdf"), width = 20, height = 15, onefile = FALSE
)
...
pdf(paste0(figures_dir,"Jericho_nutrients.pdf"),
    width = 15, height = 11, onefile = FALSE
)
nutrients_plot_line
dev.off()
...
chla_plot_full_line <- ggplot(chla_data,
 ...
 season_line+
 spring_bloom_line+
 geom_line(colour = "seagreen1",size=1.5) +
 geom_point(colour = "grey50") +
 date_scaling+
...
pdf(paste0(figures_dir,"Jericho_chl_a%03d.pdf"), width = 15, height = 11, onefile = FALSE
)
...
pdf(paste0(figures_dir,"Jericho_secchi.pdf"), width = 15, height = 11, onefile = FALSE
)

sechi_plot_full_line <- ggplot(droplevels(subset(Jericho_enviro_and_bio_long,
                                                 (variable == "Secchi_disk_disappears"))),aes(x = Date,y = as.numeric(value),
                                                                                              group = variable))+
 season_line+
 spring_bloom_line+
...
## Dissolved oxygen
pdf(paste0(figures_dir,"Jericho_dO.pdf"), width = 15, height = 11, onefile = FALSE
)

DO_plot_full_line <- ggplot(droplevels(subset(Jericho_enviro_and_bio_long,
                                                 (variable == "Dissolved_oxygen_percent"))),aes(x = Date,y = as.numeric(value),
                                                                                              group = variable))+
 season_line+
 spring_bloom_line+
 geom_line(colour = path_colour,size=1.5) +
