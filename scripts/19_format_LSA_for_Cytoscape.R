...
write.csv(good_lsa, "../results/LSA_tables/good_lsa.csv")

## maybe will have to break up script here. 
...
make_igraph_for_cytoscape <- function (lsa_network, names_amplicons) {
 ...
 graph1 <- graph.data.frame(lsa_network, directed=TRUE)
 ...
 E(graph1)$weight_LS <- lsa_network$LS
 E(graph1)$delay <- lsa_network$Delay
 ...
 write.graph(graph1,
             file=paste("../results/LSA_tables/network_LSA_",
                        names_amplicons,
                        ".graphml",
                        sep=""),
             format="graphml")
