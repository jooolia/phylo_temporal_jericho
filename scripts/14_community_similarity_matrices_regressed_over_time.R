...
plot_normalized_OTUs_by_date_and_community_dissimilarity <- function (normalized_otus, title) {
 ...
 p <-  ggplot(melted_matrix,
              aes(x=date_diff, y=Bray_curtis_dissimilarity))+
  geom_point()+
  ggtitle(title)+
...
plot_normalized_OTUs_by_date_and_community_dissimilarity(normalized_AVS_OTUs,
                                                         "AVS by date")
...
plot_normalized_OTUs_by_date_and_community_similarity <- function (normalized_otus, title) {
 ...
 p <-  ggplot(melted_matrix,
              aes(x=date_diff, y=Bray_curtis_similarity))+
...
plot_normalized_OTUs_by_date_and_community_similarity(normalized_18s_OTUs,
                                                      "18S by date") 
plot_normalized_OTUs_by_date_and_community_similarity(normalized_18s_OTUs_phytos,
                                                      "18S phytos by date")
plot_normalized_OTUs_by_date_and_community_similarity(normalized_18s_OTUs_hetero,
                                                      "18S hetero by date")
...
plot_normalized_OTUs_by_date_and_community_similarity(normalized_MPL_OTUs,
                                                      "MPL by date")
plot_normalized_OTUs_by_date_and_community_similarity(normalized_AVS_OTUs,
                                                      "AVS by date")
...
compare_beta_diversity <- function (normalized_A_OTUs, normalized_B_OTUs) {
#  normalized_A_OTUs <- normalized_18s_OTUs
 # normalized_B_OTUs <- normalized_MPL_OTUs
  ...
  test_beta <- merge(A_diss, B_diss, by=c("Var1", "Var2"))
 test_beta <- dplyr::filter(test_beta, value.x >0 & value.y >0)
  str(test_beta)

 p1 <-  ggplot(test_beta, aes(x=value.x, y=value.y))+
   geom_point(colour=point_colour)
  print(p1)
  
  print(lm_comparison <- lm(value.y ~ value.x, test_beta))
  
 p2 <-  ggplot(test_beta, aes(x=value.x, y=value.y))+
   geom_point(colour=point_colour)+
   stat_smooth(method="lm", se=FALSE)+ 
  ## problem with this in the function. If it is run  beforehand it is evaluated, but it always takes the old version. 
  geom_text(aes(x = min(value.x)+0.1, y = max(value.y)-0.1), 
...
compare_beta_diversity(normalized_18s_OTUs,normalized_MPL_OTUs)

compare_beta_diversity(normalized_16s_OTUs,normalized_gp23_OTUs)

compare_beta_diversity(normalized_16s_OTUs,normalized_MPL_OTUs)

compare_beta_diversity(normalized_18s_OTUs,normalized_MPL_OTUs)

compare_beta_diversity(normalized_18s_OTUs,normalized_16s_OTUs)

compare_beta_diversity(normalized_MPL_OTUs,normalized_gp23_OTUs)
