...
## idea to choose between distance matrices
# 
...
#  env_params_to_choose <- params_Jericho$Temperature_YSI
 ...
 print(env_otu_table_A_mantel <- mantel(env.dist,as.dist(as.matrix(SetA_diss_filt)) , method="spearman"))
 print(env_otu_table_B_mantel <-mantel(env.dist,as.dist(as.matrix(SetB_diss_filt)) , method="spearman"))
 
 partial_mantel <- mantel.partial(as.dist(as.matrix(SetA_diss_filt)), as.dist(as.matrix(SetB_diss_filt)), env.dist, method = "pearson", permutations = 999, 
                                  strata = NULL, na.rm = FALSE, parallel = getOption("mc.cores"))
 
 
 ## set up lagged correlations
 SetA_lead <-  mutate(test_beta, value.x=lead(value.x) )
 SetA_lead <- na.omit(SetA_lead)
 SetA_diss_filt_lead <- dcast(SetA_lead, Var1 ~  Var2, value.var="value.x", fill=0)
 rownames(SetA_diss_filt_lead) <- SetA_diss_filt_lead$Var1
 SetA_diss_filt_lead <- SetA_diss_filt_lead[,-1]
 
print( mantel_lagged_A <- mantel(as.dist(as.matrix(SetA_diss_filt_lead)), as.dist(as.matrix(SetB_diss_filt))))
 
SetB_lead <-  mutate(test_beta, value.y=lead(value.y) )
SetB_lead <- na.omit(SetB_lead)
SetB_diss_filt_lead <- dcast(SetB_lead, Var1 ~  Var2, value.var="value.x", fill=0)
rownames(SetB_diss_filt_lead) <- SetB_diss_filt_lead$Var1
SetB_diss_filt_lead <- SetB_diss_filt_lead[,-1]

print( mantel_lagged_B <- mantel(as.dist(as.matrix(SetA_diss_filt)), as.dist(as.matrix(SetB_diss_filt_lead))))
...
test_result <- mantel_and_partial_mantel_tests(normalized_18s_OTUs,normalized_MPL_OTUs, Env_data_for_merging )
...
table_to_hold_all_mantle <- as.data.frame(c(as.character(table_to_hold_all_mantle$env_param),names_otu_tables))
...
mantel_test_env_and_otu_tables <- function (normalized_A_OTUs,
 ...
 for (x in colnames(subset(params_Jericho, select = !(colnames(params_Jericho)) %in% "Date"))){
  ...
  mantel_results <- mantel_and_partial_mantel_tests(normalized_A_OTUs,normalized_B_OTUs, Env_data_for_merging, x )
...
table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs, normalized_MPL_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs", "normalized_MPL_OTUs" )

table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_16s_OTUs, normalized_gp23_OTUs, table_to_hold_all_mantle,"normalized_16s_OTUs", "normalized_gp23_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs_phytos, normalized_MPL_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs_phytos", "normalized_MPL_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs_hetero, normalized_MPL_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs_hetero", "normalized_MPL_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs, normalized_AVS_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs", "normalized_AVS_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs_hetero, normalized_AVS_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs_hetero", "normalized_AVS_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs_phytos, normalized_AVS_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs_phytos", "normalized_AVS_OTUs" )

table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs, normalized_16s_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs", "normalized_16s_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs_hetero, normalized_16s_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs_hetero", "normalized_16s_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs_phytos, normalized_16s_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs_phytos", "normalized_16s_OTUs" )

table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_18s_OTUs, normalized_gp23_OTUs, table_to_hold_all_mantle,"normalized_18s_OTUs", "normalized_gp23_OTUs" )

table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_MPL_OTUs, normalized_gp23_OTUs, table_to_hold_all_mantle,"normalized_MPL_OTUs", "normalized_gp23_OTUs" )

table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_MPL_OTUs, normalized_16s_OTUs, table_to_hold_all_mantle,"normalized_MPL_OTUs", "normalized_16s_OTUs" )

#table_to_hold_all_mantle <- mantel_test_env_and_otu_tables(normalized_AVS_OTUs, normalized_16s_OTUs, table_to_hold_all_mantle,"normalized_AVS_OTUs", "normalized_16s_OTUs" )
